#Projeto II Bacharelado em Ciencias da computacao
#Jorez
...
lig = 1 #Variavel que mantem o loop da maquina, nao acesivel por comando
sel = 0 #Entrada de valores no menu, inicia sempre com 0, pois depende de uma matriz

admSel=[0,0,0,0,0,0,0] #vetor/matriz do menu de comando do Administrador

#funcao administrador
def admin():
    print('#################################\n'
          'Modo Administrativo')
    #Adiciona a primero menu (apemas ao ligar o programa)
    admSel[0] = int(input('0 -> Alterar produto \n1-> Inserir produto \n2-> Sair do modo Administrativo\n'))

    while admSel[0] <2: #mantem o menu adm em loop
...
while lig == 1:  # apenas para loop
    ...
    if sel == 99:
        admin() #entrando no modo administrativo
        sel=90 #atualizando para nao entrar no modo administrador
    if 0 <= sel <= (qtProd-1):
        print(M[sel][1])
        if M[sel][3] > 0:
    ...
    input('Precione qualquer tecla!') #Deixa a visao de troco parada para conferir
